...
function drawItemsTable(doc, items, defaultTaxPercent, fonts) {
  ...
  items.forEach((it, idx) => {
    ...
    const actualPrice = Number(it.actual_unit_price !== undefined ? it.actual_unit_price : it.price) || 0;
    ...
    // Format product discount note if present
    const discVal = Number(it.discount_value) || 0;
    const discType = it.discount_type || 'none';
    let discNote = '';
    if (discVal > 0) {
      const pDiscAmt = Number(it.product_discount_amount) || 0;
      discNote = discType === 'percentage'
        ? `\nDisc: ${discVal}% (-₹ ${formatNum(pDiscAmt)})`
        : `\nDisc: ₹ ${formatNum(discVal)}/unit (-₹ ${formatNum(pDiscAmt)})`;
    }

    const descText = (it.description || '') + discNote;

    // Dynamic row height calculation (Requirement #28)
    doc.fontSize(8);
    const hDesc = doc.heightOfString(descText, { width: COLUMNS.desc.width - 4 });
    ...
    doc.text(descText, colX.desc + 2, y, { width: COLUMNS.desc.width - 4, align: COLUMNS.desc.align });
    ...
    doc.text(formatCurrency(actualPrice), colX.price + 2, y, { width: COLUMNS.price.width - 4, align: COLUMNS.price.align });
...
function drawLowerSection(doc, { items, settings, subtotal, grossSubtotal, productDiscountTotal, subtotalAfterProductDiscounts, taxPercent, sameState, calcTotals, discountType, discountPercent, discountAmount, taxableAmount, roundOff: passedRoundOff, total: passedTotal }, fonts) {
  if (doc.y > 640) {
    doc.addPage();
    doc.y = MARGIN + 10;
  }
  ...
  // Safe numeric handling for discounts
  const numGrossSubtotal = Number(grossSubtotal || 0);
  const numProductDiscountTotal = Number(productDiscountTotal || 0);
  const numSubtotalAfterProductDiscounts = Number(subtotalAfterProductDiscounts || subtotal || 0);
  const numOverallDiscountAmount = Number(discountAmount || 0);
  const numOverallDiscountPercent = Number(discountPercent || 0);

  const finalTaxable = taxableAmount !== undefined && taxableAmount !== null && Number(taxableAmount) > 0
    ? Number(taxableAmount)
    : Math.max(0, numSubtotalAfterProductDiscounts - numOverallDiscountAmount);
  ...
  if (numProductDiscountTotal > 0) {
    doc.text('Gross Subtotal', summaryX, rightY, { width: 90, align: 'left' });
    doc.text(formatCurrency(numGrossSubtotal), summaryX + 90, rightY, { width: 100, align: 'right' });
    rightY += 15;

    doc.text('Product Discount', summaryX, rightY, { width: 90, align: 'left' });
    doc.text(`- ${formatCurrency(numProductDiscountTotal)}`, summaryX + 90, rightY, { width: 100, align: 'right' });
    rightY += 15;

    doc.text('Subtotal (Post-Disc)', summaryX, rightY, { width: 90, align: 'left' });
    doc.text(formatCurrency(numSubtotalAfterProductDiscounts), summaryX + 90, rightY, { width: 100, align: 'right' });
    rightY += 15;
  } else {
  ...
  if (numOverallDiscountAmount > 0) {
    const pctStr = numOverallDiscountPercent > 0
      ? (Number.isInteger(numOverallDiscountPercent) ? numOverallDiscountPercent.toString() : numOverallDiscountPercent.toFixed(2))
      : null;
    const discountLabel = pctStr ? `Overall Disc (${pctStr}%)` : 'Overall Discount';
    doc.text(discountLabel, summaryX, rightY, { width: 90, align: 'left' });
    doc.text(`- ${formatCurrency(numOverallDiscountAmount)}`, summaryX + 90, rightY, { width: 100, align: 'right' });
    rightY += 15;
...
function buildDocument(res, { title, number, date, billTo, items, subtotal, grossSubtotal, productDiscountTotal, subtotalAfterProductDiscounts, taxPercent, settings, createdByUsername, extraDetailLines, discountType, discountPercent, discountAmount, taxableAmount, roundOff, total }) {
...
export function streamQuotationPdf(res, { quotation, items, settings }) {
  ...
  if (quotation.branch_id) {
    try {
      const branch = db.prepare(`
        SELECT b.*, f.firm_name, f.legal_name, f.gstin AS firm_gstin, f.pan AS firm_pan, f.logo AS firm_logo
        FROM branch b
        JOIN firm f ON f.id = b.firm_id
        WHERE b.id = ?
      `).get(quotation.branch_id);

      if (branch) {
        const fullAddr = [branch.address_line_1, branch.address_line_2, branch.city, branch.state, branch.pincode]
          .filter(Boolean)
          .join(', ');

        branchSettings = {
          ...settings,
          company_name: `${branch.firm_name || 'TECHNICON SERVICES'} — ${branch.branch_name}`,
          address: fullAddr || settings.address,
          phone: branch.phone || settings.phone,
          email: branch.email || settings.email,
          gstin: branch.gstin || branch.firm_gstin || settings.gstin,
          state: branch.state || settings.state,
          bank_details: branch.bank_details || settings.bank_details,
          terms_conditions: branch.terms_conditions || settings.terms_conditions,
        };
      }
    } catch (err) {
      console.error('Failed to load branch settings for quotation PDF:', err);
    }
  }
  ...
  buildDocument(res, {
    ...
    grossSubtotal: quotation.gross_subtotal,
    productDiscountTotal: quotation.product_discount_total,
    subtotalAfterProductDiscounts: quotation.subtotal_after_product_discounts,
    taxPercent: quotation.tax_percent,
    discountType: quotation.discount_type || quotation.overall_discount_type,
    discountPercent: quotation.discount_percent,
    discountAmount: quotation.discount_amount || quotation.overall_discount_amount,
