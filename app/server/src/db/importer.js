...
export async function importWorkbook(buffer, { filename, yearLabel }) {
  ...
  const companiesBefore = db.prepare(`SELECT COUNT(*) c FROM company`).get().c;
  const productsBefore = db.prepare(`SELECT COUNT(*) c FROM product`).get().c;

  const insertBatch = db.prepare(
    `INSERT INTO import_batch (filename, year_label, row_count) VALUES (?, ?, ?)`
  );
  const batchInfo = insertBatch.run(filename, yearLabel || null, rows.length);
  ...
  const findDuplicate = db.prepare(
    `SELECT 1 FROM sales_record
     WHERE IFNULL(invoice_no,'') = ? AND IFNULL(company_name,'') = ? AND IFNULL(sale_date,'') = ?
       AND IFNULL(part_no,'') = ? AND IFNULL(qty,0) = ? AND IFNULL(price,0) = ? AND IFNULL(total_amount,0) = ?
     LIMIT 1`
  );
  const insertSale = db.prepare(`
    ...
    VALUES (@sale_date, @invoice_no, @company_name, @company_id, @po_no, @state, @hsn_sac, @part_no, @product_description, @price, @qty, @total_amount, @needs_review, @review_reason, @import_batch_id)
  `);
  ...
  const tx = db.transaction(() => {
    for (const r of rows) {
      ...
      let companyId = null;
      if (companyName) {
        insertCompany.run(companyName, stateRes.value);
        companyId = getCompanyId.get(companyName).id;
      }

      if (partNo && description && !Number.isNaN(price)) {
        insertProduct.run(partNo, r.hsnSac != null ? String(r.hsnSac) : null, description, price);
      }
      ...
      insertSale.run({
        sale_date: dateRes.value,
        invoice_no: invoiceNo,
        company_name: companyName,
        company_id: companyId,
        po_no: normalizePoNo(r.poNo),
        state: stateRes.value,
        hsn_sac: r.hsnSac != null ? String(r.hsnSac) : null,
        product_description: description,
        part_no: partNo,
        price: Number.isNaN(price) ? null : price,
        qty: Number.isNaN(qty) ? null : qty,
        total_amount: Number.isNaN(totalAmount) ? null : totalAmount,
  ...
  const companiesAdded = db.prepare(`SELECT COUNT(*) c FROM company`).get().c - companiesBefore;
  const productsAdded = db.prepare(`SELECT COUNT(*) c FROM product`).get().c - productsBefore;
